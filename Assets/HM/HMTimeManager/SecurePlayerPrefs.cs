...
public class SecurePlayerPrefs : ScriptableObject
{
	...
	private static byte[] keyArray = UTF8Encoding.UTF8.GetBytes("86bU1IF9Xx343R076xW547OCP5783TcB"); // 256-AES key Replace with your own
    ...
    {
		//Log.Magenta("SecurePlayerPrefs: SetInt: " + Key + ", value: " + Value);
        PlayerPrefs.SetString(Encrypt(Key), Encrypt(Value.ToString()));
    }
    public static void SetString(string Key, string Value)
    {
		//Log.Magenta("SecurePlayerPrefs: SetString: " + Key + ", value: " + Value);
        PlayerPrefs.SetString(Encrypt(Key), Encrypt(Value));
    }
    public static void SetFloat(string Key, float Value)
    {
		//Log.Magenta("SecurePlayerPrefs: SetFloat: " + Key + ", value: " + Value);
        PlayerPrefs.SetString(Encrypt(Key), Encrypt(Value.ToString()));
    }
    public static void SetBool(string Key, bool Value)
    {
		//Log.Magenta("SecurePlayerPrefs: SetBool: " + Key + ", value: " + Value);
        PlayerPrefs.SetString(Encrypt(Key), Encrypt(Value.ToString()));
    }
    ...
    {
		bool hasKey = PlayerPrefs.HasKey(Encrypt(Key));
		//Log.Magenta("SecurePlayerPrefs: HasKey: " + Key + ", result: " + hasKey);
		return hasKey;
    }
}
